...
export default function Recruitment() {
  ...
  async function submit() {
    ...
    if (!form.prenom || !form.nom || !form.email || !form.telephone) { setSubmitError('Veuillez remplir vos nom, prénom, email et téléphone.'); return; }
                ...
                return (
                  <div key={j._id} className="rc-card" onClick={() => open(j._id)} style={{ cursor: 'pointer', borderRadius: 22, padding: '26px 26px 24px', background: 'rgba(255,255,255,0.78)', backdropFilter: 'blur(14px) saturate(150%)', WebkitBackdropFilter: 'blur(14px) saturate(150%)', border: '1px solid rgba(255,255,255,0.85)', boxShadow: '0 18px 44px -28px rgba(30,45,70,0.4)', display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 2 }}>
                      <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9c7d2c', background: 'rgba(197,160,57,0.13)', padding: '5px 10px', borderRadius: 7 }}>{j.jobType}</span>
                      <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(20,24,31,0.5)', background: 'rgba(20,24,31,0.05)', padding: '5px 10px', borderRadius: 7 }}>{j.experience}</span>
                    </div>
      ...
      {selected && (
        <section className="rc-detail" style={{ maxWidth: 880, margin: '0 auto', padding: '46px 40px 110px' }}>
          ...
          {(() => {
            ...
            if (j.experience) chips.push('Exp. ' + j.experience); if (j.educationLevel) chips.push(j.educationLevel);
